    ...
    <div class="content-wrapper">
        ...
        <div class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1 class="m-0">Senarai Lesen Memburu</h1>
                    </div><!-- /.col -->
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item">Lesen Memburu</li>
                        ...
                        <div class="card card-primary card-outline">
                            <div class="card-header">
                                <div class="row">
                                    ...
                                    <div class="col-6">
                                        <a href="/lesen/memburu/baru" class="btn btn-primary float-right"><i
    ...
    @push('script')
        <script>
            ...
            function fillDataTable() {
                ...
                //Send ajax requests
                $.ajax({
                    url: '/lesen/memburu/loadLesenDatatable',
                    type: 'POST',
                    data: {
            ...
            function addDataTableRow(response) {
                ...
                $.each(response.result, function(i, data) {
                    var viewButton = '<a href="/lesen/memburu/view/' + data.id +
                        '" type="button" class="btn btn-warning btn-xs"><i class="fa fa-eye"></i></a>';
                    var deleteButton = '<button type="button" class="btn btn-danger btn-xs btnDeleteLesen" data-id="' +
                    ...
                    if (data.status_borang_id == 5 && data.resit_am_id) {
                        generatePDF =
                            '<a type="button" target="_blank" data-toggle="tooltip" data-placement="top" title="Generate Permit" class="btn btn-default btn-sm" href="/lesen/memburu/pdf/' +
            ...
            $('#tableBorang').on("click", ".btnDeleteLesen", function() {
                if (confirm('Adakah anda pasti untuk hapuskan borang ini? Klik OK untuk hapuskan sekarang')) {
                    ...
                    //Send ajax requests
                    $.ajax({
                        url: '/lesen/memburu/postDeleteLesen',
                        type: 'POST',
                        data: {
                            _token: $('meta[name="csrf-token"]').attr('content'),
                            borang_id: row_id,
            ...
            $('#btnSaveResitAm').click(function(e) {
                ...
                //alert($("#pejabat_pembayaran_id option:selected").text());
                ...
                if ($('#resitAmForm').parsley().isValid()) {
                    ...
                    //Send ajax requests
                    $.ajax({
                        url: '/lesen/memburu/view/postUpdateResitAm',
                        type: 'POST',
            ...
            function UpdatePejabat() {
                var defaultInputID = $('#pejabat_pembayaran_id').val();
                var defaultInputText = $('#pejabat_pembayaran_id option:selected').text();
                var promptMessage = "";


                if (defaultInputText == "Pilih") { //New
                    promptMessage = "Sila masukkan pejabat pembayaran yang baru";
                    defaultInputText = "";
                } else { //Edit
                    promptMessage = "Sila ubah pejabat pembayaran berikut";
                }

                var enteredInput = prompt(promptMessage, defaultInputText);
                ...
                if (enteredInput) {
                    ...
                    $.ajax({
                        url: '/lesen/postUpdatePejabatPembayaran',
                        type: 'post',
                        data: {
                            pejabat_pembayaran_id: defaultInputID,
                            input_value: enteredInput,
                        },
                        dataType: 'JSON',
                        success: function(response) {
                            //Update select input
                            $('#pejabat_pembayaran_id').empty();
                            $('#pejabat_pembayaran_id').append(new Option("Pilih", ""));

                            $.each(response, function(i, data) {
                                $('#pejabat_pembayaran_id').append(new Option(data.pejabat, data
                                    .pejabat_pembayaran_id));
                            });
                        }
                    });
                }
            ...
            $('#tableBorang').on("click", ".btnrenewLesen", function() {
                if (confirm('Adakah anda pasti untuk memperbaharui permohonan ini?')) {
                    ...
                    //Send ajax requests
                    $.ajax({
                        url: '/lesen/memburu/postRenewPermit',
                        type: 'POST',
                        data: {
                            _token: $('meta[name="csrf-token"]').attr('content'),
                            permit_id: row_id,
