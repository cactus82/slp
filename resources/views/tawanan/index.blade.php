            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="/penguatkuasaan">Permit Haiwan Tawanan</a></li>
                ...
                <div class="card-header">
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            ...
                            <a href="/permit/haiwantawanan/baru" id="btnBaru" class="btn btn-primary float-right"><i
...
<!-- /.content-wrapper -->
@include('tawanan.resit_am_modal');
@include('tawanan.permit_file_modal');
...
<script>
    ...
    function fillDataTable() {
        ...
        //Send ajax requests
        $.ajax({
            url: '/permit/haiwantawanan/loadBorang',
            type: 'POST',
            data: {
    ...
    function addDataTableRow(response) {
        ...
        $.each(response.result, function(i, data) {
            var viewButton = '<a href="/permit/haiwantawanan/view/' + data.id +
            ...
            if (data.status_borang_id == 5 && data.resit_am_id) {
                generatePDF =
                    '<a type="button" target="_blank" data-toggle="tooltip" data-placement="top" title="Generate Permit" class="btn btn-default btn-sm" href="/permit/tawanan/pdf/' +
    ...
    $('#tablePermit').on("click", ".btnDeletePermit", function() {
        if (confirm('Adakah anda pasti untuk hapuskan borang ini? Klik OK untuk hapuskan sekarang')) {
            ...
            //Send ajax requests
            $.ajax({
                url: '/permit/haiwantawanan/postDeleteBorang',
                type: 'POST',
                data: {
                    _token: $('meta[name="csrf-token"]').attr('content'),
                    borang_id: row_id,
    ...
    $('#btnSaveResitAm').click(function(e) {
        ...
        if ($('#resitAmForm').parsley().isValid()) {
            ...
            //Send ajax requests
            $.ajax({
                url: '/permit/haiwantawanan/postUpdateResitAm',
                type: 'POST',
    ...
    $('#tablePermit').on("click", ".btnRenewPermit", function() {
        if (confirm('Adakah anda pasti untuk renew permohonan ini?')) {
            ...
            //Send ajax requests
            $.ajax({
                url: '/permit/haiwantawanan/postRenewPermit',
                type: 'POST',
                data: {
                    _token: $('meta[name="csrf-token"]').attr('content'),
                    permit_id: row_id,
